...
## WIND PARAMETERS
wind = 0 #Wind speed in m/s (positive right to left)
...
def export_plots(file_name):
    ...
    servo.export_u_delta_log(filepath + 'u_delta_log.csv')  # [instrumentation]
...
def run_sim_local():
    ...
    v_prev = 0.0
    CHUTE_DESCENT_RATE = -5.0  # m/s, safe descent speed once "deployed"

    while t <= sim_duration:
        simulation()

        # Apogee-triggered parachute: once vertical velocity flips from
        # climbing to falling, clamp descent to a safe rate. Simplified
        # stand-in for real chute drag (doesn't touch the Cd/aero model),
        # good enough to validate a full ascent-apogee-descent profile.
        if not chute_deployed and v_prev > 0 and v_glob[0] <= 0:
            chute_deployed = True
            print("\nParachute Deployed")
        if chute_deployed and v_glob[0] < CHUTE_DESCENT_RATE:
            v_glob[0] = CHUTE_DESCENT_RATE
        v_prev = v_glob[0]
        """
