...
from src.aerodynamics.drag_data import DragModel
...
class Rocket:
    ...
    def __init__(self):
        ...
        self.t_burnout = 1
        self.reynolds = 1
        # Imported Cd override (RASAero II or OpenRocket export). Leave
        # drag_model as None to keep AeroVECTOR's default empirical drag
        # build-up (friction + pressure + base drag). Set it (see
        # set_drag_model below) to replace the body Cd0 with an
        # interpolated value from the imported table each step.
    ...
    def _calculate_mach(self):
        ...
        if mach < 0.001:
            ...
            self.mach = mach
        self.beta = np.sqrt(1 - min(self.mach, 0.999)**2)
    ...
    def _calculate_cd(self, aoa):
        if self.drag_model is not None:
            # Replace AeroVECTOR's default empirical drag build-up (friction
            # + pressure + base drag) with an interpolated value from the
            # imported Cd table (RASAero II or OpenRocket export). aoa is
            # in radians internally, exported tables are in degrees.
            self.cd0 = self.drag_model.get_cd(
                mach=self.mach, aoa_deg=aoa * RAD2DEG)
            return
        self.cd0 = 0
    ...
    def set_drag_model(self, csv_path, use_power_on=False):
        """
        Load an exported Cd CSV (RASAero II or OpenRocket) and use it in
        place of AeroVECTOR's default drag build-up for all subsequent
        calls to calculate_aero_coef.

        Parameters
        ----------
        csv_path : str
            Path to a RASAero II "Export All Data" CSV, or an OpenRocket
            "Export Data" CSV with Mach number and a drag coefficient
            column selected.
        use_power_on : bool, optional
            RASAero only: use the "CD Power-On" column instead of
            "CD Power-Off". Default is False (Power-Off / coast Cd,
            RASAero's standard reference value). Has no effect on
            OpenRocket exports.
    ...
    def clear_drag_model(self):
        """Revert to AeroVECTOR's default empirical drag calculation."""
        self.drag_model = None
